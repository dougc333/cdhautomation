...
def add_cloudera_mgmt_service(api):
  ...
  #   cm.update_license(file_contents)


def add_hosts(api):
   cm=ClouderaManager(api)
   api.create_cluster("HDDTest","CDH5")
...
def add_services(api):
   """
   """  
   #check hosts are still installed
   cm=ClouderaManager(api)
   ...
   for cluster in api.get_all_clusters():
      ...
      #check to see hdfs service is available
      service = cluster.create_service("HDFS","HDFS")
